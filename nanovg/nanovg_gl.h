...
static void glnvg__checkError2(const char* str)
{
	GLenum err;
	err = glGetError();
	if (err != GL_NO_ERROR) {
...
glnvg__checkError2("create");

	vert = glCreateShader(GL_VERTEX_SHADER);

glnvg__checkError2("vert");

	frag = glCreateShader(GL_FRAGMENT_SHADER);

glnvg__checkError2("frag");

	str[2] = vshader;
	glShaderSource(vert, 3, str, 0);

glnvg__checkError2("source1");

	str[2] = fshader;
	glShaderSource(frag, 3, str, 0);

glnvg__checkError2("source2");

	glCompileShader(vert);

glnvg__checkError2("compile");
